...
def displacement_solver_vectorized(no, nc, el, ec, df, phi, psi, coh, E, nu, Fext, Km_d, ad, lim, tol):
    ...
    for k in range(lim):
        ...
        for j in range(4):
            ...
            F, dq1, dq2, dq3, sigma_id = yield_mohr_coulomb(p, q, t, snph[id], csph[id], snps[id], coh[id])
            sigma[id] = sigma_id
...
def monte_carlo_iteration(ks_soil, phi_soil, coh_soil, L, free, pile, inter, phi_pile, phi_inter, dilation_factor,
    ...
    # Darcy flow
    vee, normal_flow_rate = flow_darcy(H, no, el, ec, ks)
    ...
    return normal_flow_rate, factor_of_safety, U, evpt1, evpt2, evpt3, evpt4, sigma, ks, H
...
# function 55: display Monte Carlo results
def display_Monte_Carlo(normal_flow_rate, FoS):
...
# function 56: display single run results
def display_visuals_single_run(free, wl, pile, def_scaling, equivalent_pile_thickness, anchor_Id, no, nc, el, ec,
                               nodal_water_pressure, lip, FoS, ks, H, U, evpt1, evpt2, evpt3, evpt4, sigma,
                               active_pile):
    ...
    vee, normal_flow_rate = flow_darcy(H, no, el, ec, ks)
    normal_flow_rate = np.round(normal_flow_rate, 6)
    # Modify sigma for active pile integration points
    if active_pile == True:
        id = select_pile_integration_points(free, pile, equivalent_pile_thickness, lip)
        sigma[id] = 0.0
        pile_ID = select_pile_elements(free, pile, no, el)
    else:
        pile_ID = []
    ...
    sm4 = cm.ScalarMappable(norm=norm, cmap=cmap)
    sm4.set_array([])
    ...
    # create a colorbar
    plt.colorbar(sm4, ax=ax, orientation='vertical')
    plt.xlabel('Width [m]', fontsize=14)
    plt.ylabel('Height [m]', fontsize=14)
    plt.axis('equal')
    plt.tight_layout()
...
# Soil Input
clx = 15.0  # [m] correlation length x-dir
cly = 1.0  # [m] correlation length y-dir
E_soil = 1e5  # [kPa] Young's modulus soil
nu_soil = 0.30  # [-] Poison's ratio soil
gamma_soil = 20.0  # [kN/m3] specific weight soil
phi_soil = [20.0, 25.0]  # [deg] friction angle [min|max]
coh_soil = [15.0, 2.2]  # [kPa] cohesion [mean|standard deviation]
ks_soil = [0.01, 0.005]  # [m/d] hydraulic conductivity [mean|standard deviation]
...
num_cores = 4  # [-] Number of Cores to be used
...
gamma = specific_weight_integration_point(free, pile, inter, gamma_soil, gamma_pile, gamma_inter,
                                          equivalent_pile_thickness, ec, lip, active_pile, active_inter)
# self weight vector
...
if __name__ == '__main__':
    if mc == 1:
        ...
        results = monte_carlo_iteration(ks_soil, phi_soil, coh_soil, L, free, pile, inter, phi_pile, phi_inter,
                                        dilation_factor, psi_pile, psi_inter, coh_pile, coh_inter,
                                        equivalent_pile_thickness, lip, active_pile, active_inter, H0, fh, fs, no, nc,
                                        el, ec, df, E, nu, Km_d, ad, lim, tol)
        normal_flow_rate, factor_of_safety, U, evpt1, evpt2, evpt3, evpt4, sigma, ks, H = results
        display_visuals_single_run(free, wl, pile, def_scaling, equivalent_pile_thickness, anchor_Id, no, nc, el, ec,
                                   nodal_water_pressure, lip, factor_of_safety, ks, H, U, evpt1, evpt2, evpt3, evpt4,
                                   sigma, active_pile)
        end_time = time.time()
    else:
        start_time = time.time()
    ...
    print(f"Monte Carlo Simulation Time: {MonteCarloSimTime} seconds")
